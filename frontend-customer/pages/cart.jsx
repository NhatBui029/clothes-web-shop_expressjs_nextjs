...
const CartPage = () => {
    ...
    const handleOrder = async () => {
        ...
        if (isLoggedIn && productList.length && methodPayment == 'cod') {
            try {
        ...
        if (isLoggedIn && productList.length && methodPayment == 'zalopay') {
            try {
                const respond = await axios.post(`${backendAPI}/api/payment/create`, order);
                window.open(respond.data, '_blank');
            } catch (error) {
                console.log(err);
                swtoast.error({
                    text: "Có lỗi khi tạo đơn hàng vui lòng thử lại!"
            ...
            <div className="row">
                <div className="col-7 cart-left-section">
                    ...
                    <div className="payment">
                        ...
                        <div onClick={() => setMethodPayment('cod')}>
                            <label htmlFor="" className="payment-item w-100 border-radius d-flex align-items-center justify-content-start">
                                <div className='payment-item-radio'>
                                    <Radio checked={methodPayment == 'cod'}></Radio>
                                </div>
                        ...
                        <div onClick={() => setMethodPayment('zalopay')}>
                            <label htmlFor="" className="payment-item w-100 border-radius d-flex align-items-center justify-content-start">
                                <div className='payment-item-radio'>
                                    <Radio checked={methodPayment == 'zalopay'}></Radio>
                                </div>
                                <div className='payment-item-icon'>
                                    <FaShippingFast />
                                </div>
                                <div className="payment-item-name">
                                    <p className="">ZaloPay</p>
                                    <p className="">Thanh toán ngay !!</p>
                                </div>
                            </label>
                        </div>
                    </div>
